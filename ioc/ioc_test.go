...
func Test_AddTransientSelf_WhenServiceIsRegisteredAsSelfTransientWithoutDependencies_ThenNewInstances(t *testing.T) {
	...
	err := AddTransientSelf[*firstTestInterfaceImpl](services, NewFirstTestInterface)
	assert.Nil(t, err)

	provider, err := services.BuildServiceProvder()
	assert.Nil(t, err)

	service1, err := Resolve[*firstTestInterfaceImpl](provider)
	assert.Nil(t, err)

	service2, err := Resolve[*firstTestInterfaceImpl](provider)
	assert.Nil(t, err)
